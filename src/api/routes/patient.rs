...
use crate::application::patient::PatientAppService;
...
pub async fn create_patient(
    ...
    let service = PatientAppService::new(ctx);
...
#[utoipa::path(
    ...
    tag = "patients",
    security(("bearer_auth" = [])),
    params(("id" = String, Path, description = "患者ID")),
    responses(
...
#[get("/patients?<name>&<page>&<page_size>")]
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "创建成功", body = PatientProfileResponse),
        (status = 404, description = "患者不存在"),
    )
)]
#[post("/patients/<id>/profile", data = "<req>")]
