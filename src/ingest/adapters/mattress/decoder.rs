...
fn map_bedentry(entry: BedEntry) -> JsonValue {
    ...
    if let Some(sn) = entry.sn {
        map.insert("sn".to_string(), JsonValue::String(sn));
    }
    if let Some(d) = entry.d {
        if let Some(st) = d.st {
            map.insert("st".to_string(), JsonValue::String(st));
        }
        if let Some(hb) = d.hb {
            map.insert(
                "hb".to_string(),
                JsonValue::Number(serde_json::Number::from(hb)),
            );
        }
        if let Some(br) = d.br {
            map.insert(
                "br".to_string(),
                JsonValue::Number(serde_json::Number::from(br)),
            );
        }
        if let Some(wt) = d.wt {
            map.insert("wt".to_string(), JsonValue::Bool(wt));
        }
        if let Some(od) = d.od {
            map.insert(
                "od".to_string(),
                JsonValue::Number(serde_json::Number::from(od)),
            );
        }
        if let Some(we) = d.we {
            map.insert(
                "we".to_string(),
                JsonValue::Number(serde_json::Number::from(we)),
            );
        }
        if let Some(p) = d.p {
            if p.len() >= 2 {
                map.insert(
                    "p".to_string(),
                    JsonValue::String(format!("[{},{}]", p[0], p[1])),
                );
            }
        }
        if let Some(fv) = d.fv {
            map.insert(
                "fv".to_string(),
                JsonValue::Number(serde_json::Number::from(fv)),
            );
        }
    }
    map.insert(
        ...
        if (0xA1..=0xA7).contains(&b) {
            ...
            match key.as_str() {
                "hb" => {
                    map.insert(
                        "hb".to_string(),
                        JsonValue::Number(serde_json::Number::from(v as i32)),
                    );
                }
                "br" => {
                    map.insert(
                ...
                "od" => {
                    map.insert(
                        "od".to_string(),
                        JsonValue::Number(serde_json::Number::from(if v == 255 {
                            -1i32
                        } else {
                            v as i32
                        })),
